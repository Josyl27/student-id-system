...
function dbErr(error, fallback="Couldn't save. Check your connection and try again."){
  ...
  if(error.code === "23505") toast("Another student already has this LRN.");
...
after.template = () => {
  ...
  document.querySelectorAll("[data-img]").forEach(inp=>inp.addEventListener("change", async e=>{
    const k=e.target.dataset.img, f=e.target.files[0], err=validateImage(f,8); if(err) return toast(err);
    try{
    ...
    }catch(x){ return toast(x.message || "Couldn't read that image."); }
...
const ACTIONS = {
  ...
  async preview(id){
    ...
    openModal("ID preview", `<div class="stack">${cardPair('data-pv="1"', fullName(s))}
...
async function importCSV(file){
  ...
  if(!head.includes("student_id")) return toast("Couldn't find the LRN column.");
  if(!head.includes("last_name") && !head.includes("name")) return toast("Couldn't find the Last Name column.");
